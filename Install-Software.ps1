<#
Applies Software.winget.dsc.yaml one package at a time via `dsc resource set`,
so a single package failure (e.g. hash verification) can't abort the rest of
the run the way `dsc config set -f Software.winget.dsc.yaml` does.

If a package's set fails and the error looks like a hash-verification failure,
retries that one package with `winget install --ignore-security-hash` before
giving up on it.
#>
[CmdletBinding()]
param(
